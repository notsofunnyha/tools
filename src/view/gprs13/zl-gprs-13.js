...
// string -> string
export function checkdata(preData) {
  const data = preData.replace(/\s/g, '').toLowerCase()
...
// String ->
export function decode(preData) {
  const data = preData.replace(/\s/g, '').toLowerCase()
  const result = {}
  const cmdCode = getCmdCode(data)

  if (cmdCode === '28') {
    result['28副参数信息'] = getCmd28Result(data)
  } else if (cmdCode === '17') {
    result['报警信息'] = getCmd17Alarm(data)
  }
  ...
  if (gpsData) {
    ...
    resultPosition['终端位置时间'] = dayjs(`${year}-${month}-${day} ${hour}:${minute}:${second}`).add(8, 'hour').format('YYYY-MM-DD HH:mm:ss')
...
function getCmd17Alarm(data) {
  ...
  if (cmd17Alarm === '10101010') {
    ...
    resultAlarm['报警值'] = alarmList[alarmValue] || alarmValue
  }
  return resultAlarm
}
...
function getCmd28Result(data) {
  ...
  const cmd28Result = {}
  cmd28Result['主电源电压'] = `${(skipnTakemToInt16(0, 4, cmd28content) / 10).toFixed(1)} V`
  cmd28Result['备用电池电压'] = `${(skipnTake2ToInt16(4, cmd28content) / 10).toFixed(1)} V`
  cmd28Result['终端内部温度'] = `${skipnTake2ToInt16(6, cmd28content) - 60} ℃`
  cmd28Result['主电休眠上报间隔'] = `${skipnTake2ToInt16(8, cmd28content)} h`
  cmd28Result['备电休眠上报间隔'] = `${skipnTake2ToInt16(10, cmd28content)} h`
  cmd28Result['ACC ON总累计时间'] = `${skipnTakemToInt16(12, 8, cmd28content)} min`
  cmd28Result['GPS终端总通电时间'] = `${skipnTakemToInt16(20, 8, cmd28content)} min`
  cmd28Result['开盖次数'] = skipnTake2ToInt16(28, cmd28content)
  cmd28Result['拔GPS天线次数'] = skipnTake2ToInt16(30, cmd28content)
  cmd28Result['拔SIM卡次数'] = skipnTake2ToInt16(32, cmd28content)
  cmd28Result['GSM信号强度'] = skipnTake2ToInt16(34, cmd28content)
  return cmd28Result
}
